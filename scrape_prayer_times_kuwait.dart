/// Kuwait Prayer Times Scraper — salatcalendar.com
///
/// Switches cities using the official select_city/{id} endpoint, then fetches
/// the full-year calendar via get_year.
///
/// City IDs sourced from: salatcalendar.com/index.php/countries/cities/114
///
/// Usage (no packages needed — pure dart:io):
///   dart scrape_prayer_times_kuwait.dart
///
/// Output:
...
const String kOutputFile = 'assets/csv/kuwait_prayer_times_2026.csv';
...
//
// cityId — salatcalendar.com ID from /index.php/countries/cities/114
// ---------------------------------------------------------------------------

const List<List<String>> kKuwaitCities = [
  ['Kuwait City',      'مدينة الكويت',    '29.3759', '47.9774', '5852'],
  ['Hawalli',          'حولي',            '29.3325', '48.0286', '16487'],
  ['Al Farwaniya',     'الفروانية',       '29.2775', '47.9581', '5850'],
  ['Al Salmiya',       'السالمية',        '29.3347', '48.0764', '5848'],
  ['Al Ahmadi',        'الأحمدي',         '29.0769', '48.0838', '11273'],
  ['Al Jahra',         'الجهراء',         '29.3375', '47.6581', '17579'],
  ['Sabah Al Salem',   'صباح السالم',     '29.2667', '48.0667', '5849'],
  ['Al Fahaheel',      'الفحيحيل',        '29.0833', '48.1333', '5851'],
  ['Al Fintas',        'الفنطاس',         '29.1667', '48.1167', '5858'],
  ['Al Manqaf',        'المنقف',          '29.1000', '48.1333', '5855'],
  ['Al Mahboula',      'المهبولة',        '29.1333', '48.1333', '5860'],
  ['Salwa',            'سلوى',            '29.2833', '48.0833', '5854'],
  ['Bayan',            'بيان',            '29.3000', '48.0500', '5857'],
  ['Al Riqqa',         'الرقة',           '29.1500', '48.1000', '5853'],
  ['Al Shamiya',       'الشامية',         '29.3500', '47.9667', '5862'],
  ['Al Funaitees',     'الفنيطيس',        '29.2333', '48.1167', '5865'],
  ['Al Wafra',         'الوفرة',          '28.6167', '47.9333', '5863'],
  ['Al Zour',          'الزور',           '28.7333', '48.3000', '5864'],
  ['Al Dasma',         'الدسمة',          '29.3500', '48.0000', '14343'],
  ['Al Sulaibekhat',   'الصليبخات',       '29.3333', '47.9000', '16250'],
];
...
class HttpSession {
  ...
  Future<bool> selectCity(String cityId) async {
    try {
      ...
      _applyHeaders(req,
          referer: 'https://$kHost/index.php/countries/cities/114');
      final res = await req.close();
      _storeCookies(res);
      await res.drain<void>();
...
Future<void> main() async {
  print('====================================================');
  print('  Kuwait Prayer Times Scraper — 2026');
  ...
  for (final city in kKuwaitCities) {
    final name       = city[0];
    final cityId     = city[4];
    print('--- $name ---');
  ...
  print('====================================================');
  print('  Cities OK     : $ok / ${kKuwaitCities.length}');
