...
class NotificationChannelResolver {
  ...
  // v2 = the sounded pre-adhan channel for a custom «اقتراب الصلاة» sound. The
  // silent v1 above stays for the 'silent' selection (a channel's sound is
  // immutable, so a sounded variant must be a fresh id).
  static const preAdhanCustomPrefix = 'prayer_reminder_v2_custom_';
  ...
  /// Resolves the per-prayer pre-adhan reminder channel. `'silent'` keeps the
  /// legacy soundless [preAdhan] channel; a `custom:<file>` selection maps to a
  /// sounded `prayer_reminder_v2_custom_<stem>` channel, which the engine
  /// registers from the shared [AppSettings.customAdhans] list at sync time.
  ({String channelId, String? contentUri}) resolvePreAdhan(
    AppSettings s,
    String prayerKey,
  ) {
    ...
    if (fileName != null) {
      ...
      if (entry.isNotEmpty && entry.first.contentUri.isNotEmpty) {
        final stem = _stripExtension(fileName);
        return (
          channelId: '$preAdhanCustomPrefix$stem',
          contentUri: entry.first.contentUri,
        );
      }
    }
    return (channelId: preAdhan, contentUri: null);
  }
