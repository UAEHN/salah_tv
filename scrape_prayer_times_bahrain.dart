/// Bahrain Prayer Times Scraper — salatcalendar.com
///
/// Switches cities using the official select_city/{id} endpoint, then fetches
/// the full-year calendar via get_year.
///
/// City IDs sourced from: salatcalendar.com/index.php/countries/cities/17
///
/// Usage (no packages needed — pure dart:io):
///   dart scrape_prayer_times_bahrain.dart
///
/// Output:
...
const String kOutputFile = 'assets/csv/bahrain_prayer_times_2026.csv';
...
//
// cityId — salatcalendar.com ID from /index.php/countries/cities/17
// ---------------------------------------------------------------------------

const List<List<String>> kBahrainCities = [
  ['Manama',        'المنامة',       '26.2285', '50.5860', '907'],
  ['Muharraq',      'المحرق',        '26.2572', '50.6119', '908'],
  ['Isa City',      'مدينة عيسى',    '26.1736', '50.5478', '910'],
  ['Sitra',         'سترة',          '26.1547', '50.6189', '911'],
  ['Jid Haffs',     'جد حفص',        '26.2186', '50.5244', '912'],
  ['Al Hadd',       'الحد',          '26.2453', '50.6544', '913'],
  ['Dar Kulaib',    'دار كليب',      '26.1667', '50.5000', '909'],
  ['Al Rifaa',      'الرفاع',        '26.1300', '50.5550', '19387'],
  ['Al Malakiyah',  'المالكية',      '26.1000', '50.4833', '20275'],
  ['Hamad City',    'مدينة حمد',     '26.1167', '50.5000', '15148'],
  ['Shahrakkan',    'شهركان',        '26.1500', '50.5167', '18567'],
];
...
class HttpSession {
  ...
  Future<bool> selectCity(String cityId) async {
    try {
      ...
      _applyHeaders(req,
          referer: 'https://$kHost/index.php/countries/cities/17');
      final res = await req.close();
      _storeCookies(res);
      await res.drain<void>();
...
Future<void> main() async {
  print('====================================================');
  print('  Bahrain Prayer Times Scraper — 2026');
  ...
  for (final city in kBahrainCities) {
    final name       = city[0];
    final cityId     = city[4];
    print('--- $name ---');
  ...
  print('====================================================');
  print('  Cities OK     : $ok / ${kBahrainCities.length}');
